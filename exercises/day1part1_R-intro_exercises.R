...
gapminder2 <- read_csv("data/gapminder5.csv")
...
sort(table(gapminder07$continent), decreasing = T)
...
head(gapminder07[order(gapminder07$pop, decreasing = T),], 5)
...
gapminder07$lifeExp_round <- round(gapminder07$lifeExp, 0)
...
high_group <- gapminder07$gdpPercap[gapminder07$lifeExp_highlow == "high"]
low_group <- gapminder07$gdpPercap[gapminder07$lifeExp_highlow == "low"]

(t1 <- t.test(high_group, low_group))
...
hist(gapminder07$lifeExp, breaks = 50)

#### Scatterplots ####

# Create a scatterplot with `lifeExp` on the y-axis and `gdpPercap` on the x-axis.
plot(lifeExp ~ gdpPercap, data = gapminder07)
plot(lifeExp ~ log10(gdpPercap), data = gapminder07)

# Add a title and axis labels.
plot(lifeExp ~ gdpPercap, data = gapminder07,
     main = "Relationship between Life Expectancy and GDP/Capita",
